 ...
 * @author LinX
 */
public class CrowdtruthMetrics {
    private static final Logger LOG = LoggerFactory.getLogger( CrowdtruthMetrics.class );
    ...
    private CrowdtruthMetrics( final Set<MediaUnit> mediaUnits ) {
        ...
        mediaUnits.stream().flatMap( u -> u.getMediaVector().getResult().keySet().stream() ).forEach( u ->
                annotations.computeIfAbsent( u.getId().getName(), k -> Sets.newHashSet() ).add( u ) );
        ...
        this.workerQualityScores = new TreeMap<>( mediaUnits.stream().flatMap( u -> u.getWorkers().stream() )
                                                            .distinct().collect
                        ( Collectors.toMap( Function.identity(), w -> 1.0 ) ) );
    }
    ...
    private MetricsScores calculate( final boolean closedTask ) {
        while (true) {
            ...
            if (closedTask) {
                ...
                LOG.info( "AQS: " + newAnnotationQualityScores.entrySet().stream().collect( ImmutableMap
            ...
            LOG.info( "UQS: {}", newMediaUnitQualityScores.entrySet().stream().collect( ImmutableMap
            ...
            LOG.info( "WQS: {}", newWorkerQualityScores.entrySet().stream().collect( ImmutableMap
                    .toImmutableMap( a -> a.getKey().getId(), Entry::getValue ) ) );
        ...
        this.mediaUnitQualityScores.keySet()
                                   .forEach( m ->
                                           m.getMediaVector().getResult().keySet()
                                            .forEach( a -> mediaUnitAnnotationScores.put( a,
    ...
    private double workerMediaUnitAgreement( final Worker worker ) {
        ...
        worker.getAnnotatedMediaUnits().forEach( annotatedMediaUnit -> {
            ...
            final double weightedCosineScore = weightedCosineScore( worker.getWorkerVector( annotatedMediaUnit )
                                                                          .getAnnotationResults(),
                    annotatedMediaUnit.getMediaUnitVectorExcludingWorker( worker ) );
    ...
    private double weightedCosineScore( final ResultVector vector1, final ResultVector vector2 ) {
        ...
        vector1.getResult().forEach( ( ma, w1Ar ) -> {
            ...
            final Double aqs = this.annotationQualityScores.entrySet().stream().filter(
                    a -> a.getKey().getName().equals(
                            ma.getId().getName() ) ).findFirst().get().getValue();
    ...
    public static MetricsScores calculateClosed( final Set<MediaUnit> mediaUnits ) {
        return new CrowdtruthMetrics( mediaUnits ).calculate( true );
    }

    public static MetricsScores calculateOpen( final Set<MediaUnit> mediaUnits ) {
        return new CrowdtruthMetrics( mediaUnits ).calculate( false );
    }
